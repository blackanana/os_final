...
{
    cout << "Addrspace => SwapHeader" << endl;
...
//	cout << "number of pages of " << fileName<< " is "<<numPages<<endl;
    ...
    // ASSERT(numPages <= NumPhysPages);        // check we're not trying
                        ...
                        // virtual memory
    ASSERT(numPages <= kernel->availablePhysicalPages());
    int i = 0, j = 0;
    pageTable = new TranslationEntry[NumPhysPages];

    for(i = 0; i < numPages; i++){
        pageTable[i].virtualPage = i;
        while(kernel->PhysicalPageUsed[j] == TRUE)
            j++;
        kernel->PhysicalPageUsed[j] = TRUE;
...
// then, copy in the code and data segments into memory
    ...
    DEBUG(dbgMLFQ, "[AddrSpace::Load over] Tick [" << stats->totalTicks << "]: Thread [" << kernel->currentThread->getID() << "]");
